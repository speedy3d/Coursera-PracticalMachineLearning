...
```{r echo=TRUE}
inTrain <- createDataPartition(y=clean_training_data$classe, p=0.7, list=FALSE)
training <- clean_training_data[inTrain,]
testing <- clean_training_data[-inTrain,]
dim(training)
dim(testing)
```
...
We will perform cross-validation on the test set. We shoudl have a fairly small out of sample error as the random forest method should be very accurate. We will then output a confusion matrix of the relevant statistics.

```{r echo=TRUE}
predict_fit <- predict(model_fit, cross)
```
